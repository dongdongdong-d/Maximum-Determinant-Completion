...
void schur_complement(gsl_matrix *M,gsl_matrix *M_remained,gsl_matrix *M_reducer,int *reduced_idx,int *full_idx, int dim_remained,int dim_full){
	int col,row,i,j;
	double temp;
...
void MCS_seach_chordal_check(int* alpha,int* alpha_inv,int** adj,int dim_remained,int* num_fill_in, int* fill_in){
	...
	for(current_order = dim_remained-1;current_order>=0;current_order--){
		...
		while(current_weight_level>0){
			...
			if(is_level_active){break;}else{current_weight_level = current_weight_level - 1;}
		}
	}
...
void BK(int* R,int* P,int* X,int** adj,int length,int** R_res,int* count){
	...
	int  i,v,u,u_nbs,u_pivot,u_pivot_nbs;
	int* R_new = NULL;
	int* P_new = NULL;
	int* X_new = NULL;
	...
	if(P_sum == 0 && X_sum==0){
		...
		*count = *count+1;
		...
		for(u = 0;u<length;u++){
			if(P[u] == 1 || X[u] == 1){
				u_nbs = 0;
				for(i = 0;i<length;i++){u_nbs = adj[u][i]+u_nbs;}
				if(u_nbs>u_pivot_nbs){u_pivot = u;u_pivot_nbs = u_nbs;}
			}
		}
		for(v =0;v<length;v++){
			if(P[v] == 1 && adj[u_pivot][v] ==0){
			R_new = malloc(sizeof(int)*length);
			P_new = malloc(sizeof(int)*length);
			X_new = malloc(sizeof(int)*length);
				for(i=0;i<length;i++){
					R_new[i] = R[i];
					if(P[i] == 1 && adj[v][i]==1){P_new[i] = 1;}else{P_new[i] = 0;}
					if(X[i] == 1 && adj[v][i]==1){X_new[i] = 1;}else{X_new[i] = 0;}
				}
				R_new[v] = 1;
...
void guess_fill_in(gsl_matrix*M_target,int col,int row,int* nei,int**adj){
	...
	for(current_clique = 0;current_clique<R_count;current_clique++){
		dim_clique = 0;
		for(i = 0;i<num_nei;i++){if(R_res[current_clique][i]==1){dim_clique = dim_clique +1;}}
		dim_sub = dim_clique + 2;
		current_clique_idx = malloc(dim_sub*sizeof(int));
		j = 0;
		for(i = 0;i<num_nei;i++){if(R_res[current_clique][i]==1){current_clique_idx[j] = nei[i];j++;}}
		...
		if(delta>0){
			if(a<0){
				...
				if(left_new>left){left = left_new;}
				if(right_new<right){right = right_new;}
			}else{
				left_new  = (-b-sqrt(delta))/a/2;
				right_new = (-b+sqrt(delta))/a/2;
				if(left_new>left){left = left_new;}
...
void guess_fill_in2(gsl_matrix*M_target,int col,int row,int* nei,int *nnbs,int**adj){
	...
	for(i = 0;i<num_nei;i++){
		nnbs[i] = 0;
		for(j = 0;j<num_nei;j++){nnbs[i] = nnbs[i] + adj[nei[i]][nei[j]];}
	...
	while(minimum_nbs<num_nei){
		for(j = minimum_nbs_idx;j<num_nei;j++){nei[j] = nei[j+1];}
		num_nei = num_nei - 1;
		minimum_nbs = num_nei;
		for(i = 0;i<num_nei;i++){
			nnbs[i] = 0;
	...
	while(df_norm_now>tol){
		...
		for ( i = 0; i < empty_length; i++) {
			...
			for ( j = i; j < empty_length; j++) {
				...
				x_temp = 2*(gsl_matrix_get(workspace,rowi,rowj)*gsl_matrix_get(workspace,coli,colj) + gsl_matrix_get(workspace,rowi,colj)*gsl_matrix_get(workspace,coli,rowj));
			...
			if(dim_remained == 3){
				...
				if(num_fill_in>0){
					...
					if(gsl_linalg_cholesky_decomp1(workspace_chol) == 0){
						...
						gsl_matrix_memcpy(workspace_chol, M_remained);
						if(gsl_linalg_cholesky_decomp1(workspace_chol) == 0){optim_newton(M_remained,workspace_chol,empty_length,empty_idx,100,1e-5);}
						else{exit(1);}
					}
